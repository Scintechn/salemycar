...
export default async function InterestPage() {
  ...
  const finalPrice = priceWithCode(listing);
    ...
    <main className="flex-1">
      <div className="mx-auto max-w-xl px-6 py-12">
        ...
        <h1 className="mt-4 text-2xl font-semibold tracking-tight md:text-3xl">
          Tenho interesse no {listing.title}
        </h1>
        ...
        {referrerName ? (
          <div className="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
            ...
            <strong>{formatEUR(listing.buyer_discount)}</strong> — preço final{" "}
          ...
          <div className="mt-6 rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-700">
            ...
            <strong>{formatEUR(listing.buyer_discount)}</strong> sobre o preço
            de tabela.
          </div>
        )}
